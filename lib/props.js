...
export async function fetchPlayerProps(apiKey, games) {
  ...
  for (const game of upcoming) {
    ...
    try {
      ...
      if (!res.ok) { console.warn(`[Props] HTTP ${res.status} for game ${game.id}`); continue; }
      const data = await res.json();
      ...
      console.log(`[Props] ${game.away_team} @ ${game.home_team} — quota: ${res.headers.get("x-requests-remaining")}`);
