...
export async function fetchPlayerStats() {
  ...
  // BDL enrichment — inline, max 4 pairs of calls (search + season_averages)
  if (process.env.BALLDONTLIE_API_KEY) {
    let bdlCalls = 0;
    for (const [nameKey, pData] of Object.entries(stats)) {
      if (bdlCalls >= 4) break;
      const s = pData.season;
      if (s && s.pts > 0 && s.reb > 0 && s.ast > 0) continue;
      try {
        ...
        if (!avgRes?.data?.length) continue;
        bdlCalls++;
        ...
        stats[nameKey].season = {
          ...
          min:    parseFloat(avg.min) || s?.min || 0,
          gp:     avg.games_played || s?.gp || 0,
          pra:    (avg.pts||0)+(avg.reb||0)+(avg.ast||0) || s?.pra || 0,
          source: "balldontlie",
        };
        console.log(`[BDL] Enriched ${pData.name} (call ${bdlCalls})`);
      } catch(e) { console.warn(`[BDL] Enrich failed for ${pData.name}:`, e.message); }
    }
    console.log(`[BDL] Enrichment complete — ${bdlCalls} API calls made`);
  } else {
