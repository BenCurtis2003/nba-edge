...
// Free tier: 5 requests per minute (1 per 12 seconds)
const bdlRequestTimestamps = [];
const BDL_MAX_PER_MINUTE = 5;
const BDL_BASE_URL = "https://api.balldontlie.io/v1";

async function bdlFetch(endpoint) {
  if (!process.env.BALLDONTLIE_API_KEY) {
    console.warn("[BDL] API key not set — skipping BallDontLie enrichment");
    return null;
  }
  try {
    const now = Date.now();
    ...
    // If at limit, wait until the oldest request ages out
    if (bdlRequestTimestamps.length >= BDL_MAX_PER_MINUTE) {
      const waitMs = 60000 - (now - bdlRequestTimestamps[0]) + 200; // +200ms buffer
      console.log(`[BDL] Rate limit reached — waiting ${waitMs}ms`);
      await new Promise(r => setTimeout(r, waitMs));
    }
    bdlRequestTimestamps.push(Date.now());
    const res = await fetch(`${BDL_BASE_URL}${endpoint}`, {
      headers: { "Authorization": process.env.BALLDONTLIE_API_KEY },
      cache: "no-store",
    });
    if (!res.ok) {
      console.warn(`[BDL] HTTP ${res.status} for ${endpoint}`);
      return null;
    }
    return await res.json();
  } catch(e) {
    console.warn(`[BDL] Fetch failed for ${endpoint}:`, e.message);
    return null;
  }
}

async function bdlGetPlayerSeasonAvg(playerName) {
  // Check KV cache first — 6 hour TTL
  const cacheKey = `nba_edge:bdl_player:${playerName.toLowerCase().replace(/\s+/g, "_")}`;
  try {
    const cached = await kv.get(cacheKey);
    if (cached) {
      console.log(`[BDL] Cache hit for ${playerName}`);
      return cached;
    }
  } catch(e) {}
  ...
  const searchData = await bdlFetch(`/players?search=${searchName}&per_page=5`);
  ...
  const player = searchData.data.find(p => {
    ...
    return full === playerName.toLowerCase() ||
      full.includes(playerName.toLowerCase().split(" ")[1] || "");
  }) || searchData.data[0];

  if (!player) return null;

  // Get current season averages (BDL uses start year of season)
  const season = new Date().getFullYear();
  const avgData = await bdlFetch(`/season_averages?season=${season - 1}&player_ids[]=${player.id}`);
  if (!avgData?.data?.length) return null;
  ...
  // Cache for 6 hours
  try { await kv.set(cacheKey, result, { ex: 21600 }); } catch(e) {}
  console.log(`[BDL] Fetched and cached stats for ${playerName}`);
  return result;
}
...
export async function fetchPlayerStats() {
  ...
  try {
    ...
    if (!sbRes.ok) return { stats, bdlCallsMade: 0 };
  ...
  // BDL enrichment — only for players with incomplete ESPN data, max 4 calls per run
  let bdlCallsMade = 0;
  const BDL_MAX_CALLS = 4;
  for (const [playerName, playerData] of Object.entries(stats)) {
    if (bdlCallsMade >= BDL_MAX_CALLS) break;
    // Only enrich if ESPN data is missing key fields
    const espnSeason = playerData.season;
    if (espnSeason && espnSeason.pts && espnSeason.reb && espnSeason.ast) continue;
    const bdlStats = await bdlGetPlayerSeasonAvg(playerName);
    if (!bdlStats) continue;
    bdlCallsMade++;
    // Merge BDL into existing playerData — ESPN takes priority
    stats[playerName] = {
      ...playerData,
      season: {
        pts:         espnSeason?.pts         || bdlStats.pts,
        reb:         espnSeason?.reb         || bdlStats.reb,
        ast:         espnSeason?.ast         || bdlStats.ast,
        tpm:         espnSeason?.tpm         || bdlStats.fg3m,
        stl:         espnSeason?.stl         || bdlStats.stl,
        blk:         espnSeason?.blk         || bdlStats.blk,
        min:         espnSeason?.min         || parseFloat(bdlStats.min) || 0,
        gamesPlayed: espnSeason?.gamesPlayed || bdlStats.games_played,
        source:      espnSeason?.pts ? "espn" : "balldontlie",
      },
    };
    console.log(`[BDL] Enriched ${playerName} (${bdlCallsMade}/${BDL_MAX_CALLS})`);
  }
  console.log(`[BDL] Enrichment complete — ${bdlCallsMade} calls made`);

  return { stats, bdlCallsMade };
}
...
const SIGNAL_WEIGHTS = {
  edgeStrength:     0.25,  // How big is the EV edge
  l5Form:           0.20,  // Recent form vs the line
  seasonVsLine:     0.15,  // Season average vs prop line
  matchupDefense:   0.15,  // Opponent's defensive rating for this stat
  bookConsensus:    0.10,  // How many books agree on the line
  homeAwayFactor:   0.08,  // Home/away performance split
  paceFactor:       0.07,  // Game pace effect on counting stats
};

function scorePropConviction({ edge, trueProb, allLines, side, line, marketKey,
  playerStats, defenseStats, isHome }) {

  const signals = buildSignalBreakdown({ edge, trueProb, allLines, side, line,
    marketKey, playerStats, defenseStats, isHome });
