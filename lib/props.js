...
async function bdlFetch(endpoint) {
  ...
  try {
    ...
    if (bdlRequestTimestamps.length >= 5) {
    ...
    for (const [nameKey, pData] of Object.entries(stats)) {
      ...
      try {
        ...
        stats[nameKey].season = {
          pts:  s?.pts  > 0 ? s.pts  : (avg.pts  || 0),
          reb:  s?.reb  > 0 ? s.reb  : (avg.reb  || 0),
          ast:  s?.ast  > 0 ? s.ast  : (avg.ast  || 0),
          tpm:  s?.tpm  > 0 ? s.tpm  : (avg.fg3m || 0),
          min:  s?.min  > 0 ? s.min  : parseFloat(avg.min || "0"),
          gp:   s?.gp   > 0 ? s.gp   : (avg.games_played || 0),
          pra:  (s?.pts > 0 ? s.pts : avg.pts||0) + (s?.reb > 0 ? s.reb : avg.reb||0) + (s?.ast > 0 ? s.ast : avg.ast||0),
          source: s?.pts > 0 ? "espn" : "balldontlie",
        };
