...
export async function fetchAndCacheTeamStats(kvSave) {
  ...
  try {
    // ── Method 1: ESPN standings endpoint (most reliable for full season records) ──
    const standingsRes = await fetch(
      "https://site.web.api.espn.com/apis/v2/sports/basketball/nba/standings?season=2025",
      { cache: "no-store" }
    );
    ...
    if(standingsRes.ok) {
      ...
      for(const conf of conferences) {
        ...
        for(const entry of entries) {
          ...
          if(!name) continue;
          // stats array: find wins, losses, homeRecord, roadRecord, last10
          const statMap = {};
          for(const s of (entry.stats || [])) {
            statMap[s.name] = s.value ?? s.displayValue;
          }
          const wins    = parseInt(statMap.wins || statMap.W || 0);
          const losses  = parseInt(statMap.losses || statMap.L || 0);
          // Home/road from displayValue like "23-8"
          const homeRec = (statMap.Home || statMap.homeRecord || "0-0").toString().split("-").map(Number);
          const awayRec = (statMap.Road || statMap.roadRecord || statMap.Away || "0-0").toString().split("-").map(Number);
          const l10Rec  = (statMap.Last10 || statMap.last10 || statMap.L10 || "0-0").toString().split("-").map(Number);
          const streak  = (statMap.streak || statMap.Streak || "0").toString();
          stats[name] = {
            wins, losses,
            homeWins: homeRec[0]||0, homeLosses: homeRec[1]||0,
            awayWins: awayRec[0]||0, awayLosses: awayRec[1]||0,
            last10Wins: l10Rec[0]||0, last10Losses: l10Rec[1]||0,
            streak,
          };
        }
      }
      console.log(`[ESPN Standings] Got ${Object.keys(stats).length} teams`);
    }
  } catch(e) {
    console.warn("[ESPN Standings] failed:", e.message);
  }

  // ── Method 2: Scoreboard — gets today's games with team records embedded ──
  try {
      ...
      for(const event of (sbData.events || [])) {
        for(const comp of (event.competitions || [])) {
          for(const competitor of (comp.competitors || [])) {
            ...
            if(!name) continue;
            // Scoreboard embeds current record as "record" array
            const records = competitor.records || [];
            const overall = records.find(r => r.type === "total" || r.name === "overall");
            const home    = records.find(r => r.type === "home"  || r.name === "Home");
            const away    = records.find(r => r.type === "road"  || r.name === "Away" || r.name === "Road");
            if(overall && !stats[name]) {
              ...
              stats[name] = { wins:w||0, losses:l||0, homeWins:hw||0, homeLosses:hl||0, awayWins:aw||0, awayLosses:al||0, last10Wins:0, last10Losses:0, streak:"0" };
            } else if(overall && stats[name]) {
              // Supplement standings data with scoreboard home/away if missing
              const [hw,hl] = (home?.summary || "0-0").split("-").map(Number);
              const [aw,al] = (away?.summary || "0-0").split("-").map(Number);
              if(hw > 0) { stats[name].homeWins = hw; stats[name].homeLosses = hl; }
              if(aw > 0) { stats[name].awayWins = aw; stats[name].awayLosses = al; }
            }
          }
        }
      }
      console.log(`[ESPN Scoreboard] Supplemented with records from scoreboard`);
    }
  } catch(e) {
    console.warn("[ESPN Scoreboard] failed:", e.message);
  }

  // ── Fallback to hardcoded if ESPN totally failed ──
  if(Object.keys(stats).length < 5) {
    console.warn("[ESPN] All fetches failed — using STANDINGS_FALLBACK");
    return null;
  }
...
export async function buildConvictionPlays(games, mlWeights = null, teamStats = null) {
  ...
  for(const game of games) {
    ...
    for(const [team, opp, record, oppRecord, isHome] of [
      ...
      // ── Rich signal scoring using live ESPN stats ──────────────────────────
      const total2 = record.wins + record.losses || 1;
      const oppTotal2 = oppRecord.wins + oppRecord.losses || 1;

      // Win rate signal — scaled 35-95
      const winRateScore = Math.round(35 + winPct * 60);

      // Record edge — actual win% differential
      const winPctDiff = winPct - oppWinPct;
      const recordEdgeScore = Math.round(50 + winPctDiff * 120);

      // Home/Away factor — use actual home/away records if available
      let homeScore;
      if(isHome && record.homeWins != null) {
        const hTotal = (record.homeWins + record.homeLosses) || 1;
        const hWinPct = record.homeWins / hTotal;
        homeScore = Math.round(40 + hWinPct * 55);
      } else if(!isHome && record.awayWins != null) {
        const aTotal = (record.awayWins + record.awayLosses) || 1;
        const aWinPct = record.awayWins / aTotal;
        homeScore = Math.round(30 + aWinPct * 50);
      } else {
        homeScore = isHome ? 68 : 42;
      }

      // Last 10 games form — hot/cold streak signal
      let formScore = 55;
      if(record.last10Wins != null) {
        const l10 = record.last10Wins / ((record.last10Wins + record.last10Losses) || 1);
        formScore = Math.round(30 + l10 * 70);
      }

      // Opponent form — better opponent = lower score
      let oppFormScore = 55;
      if(oppRecord.last10Wins != null) {
        const oppL10 = oppRecord.last10Wins / ((oppRecord.last10Wins + oppRecord.last10Losses) || 1);
        oppFormScore = Math.round(30 + oppL10 * 70);
      }
      // ATS proxy: if we're the better team at home, ATS record likely favorable
      const atsScore = Math.round(45 + winPctDiff * 60 + (isHome ? 8 : -5));

      // Pace/matchup: use last game point differential as proxy if available
      let paceScore = 52;
      if(record.lastGameContext) {
        const lastMargin = parseInt(record.lastGameContext.score||0) - parseInt(record.lastGameContext.oppScore||0);
        paceScore = Math.round(50 + Math.max(-20, Math.min(20, lastMargin)) * 1.2);
      }

      const signals = [
        { key:"winRate",   label:"Season Win Rate",      weight: weights.winRate,   score: Math.max(25,Math.min(98,winRateScore)) },
        { key:"netRating", label:"Record vs Opponent",   weight: weights.netRating, score: Math.max(25,Math.min(98,recordEdgeScore)) },
        { key:"rest",      label:"Recent Form (L10)",    weight: weights.rest,      score: Math.max(25,Math.min(98,formScore)) },
        { key:"ats",       label:"ATS Tendency",         weight: weights.ats,       score: Math.max(25,Math.min(98,atsScore)) },
        { key:"home",      label:"Home/Away Record",     weight: weights.home,      score: Math.max(25,Math.min(98,homeScore)) },
        { key:"h2h",       label:"Opponent Form (L10)",  weight: weights.h2h,       score: Math.max(25,Math.min(98, 100 - oppFormScore)) },
        { key:"pace",      label:"Point Differential",   weight: weights.pace,      score: Math.max(25,Math.min(98,paceScore)) },
      ];
