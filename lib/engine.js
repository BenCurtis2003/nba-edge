...
export async function fetchAndCacheTeamStats(kvSave) {
  // Start with hardcoded fallback so we ALWAYS have 30 teams
  const stats = {};
  for(const [name, rec] of Object.entries(STANDINGS_FALLBACK)) {
    stats[name] = { ...rec,
      homeWins: Math.round(rec.wins * 0.56), homeLosses: Math.round(rec.losses * 0.44),
      awayWins: Math.round(rec.wins * 0.44), awayLosses: Math.round(rec.losses * 0.56),
      last10Wins: 5, last10Losses: 5, streak: "0",
    };
  }
