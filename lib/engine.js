...
// ── RICH TEAM STATS CACHE ────────────────────────────────────────────────────
// Fetches W/L, home/away splits, last10, streaks from ESPN in ONE call
// Cached in module scope for the engine run, persisted to KV between runs
...
export async function fetchAndCacheTeamStats(kvSave) {
  try {
    ...
    for(const t of (data.sports?.[0]?.leagues?.[0]?.teams || [])) {
      ...
      const overall = getR("total");
      const home    = getR("home");
      const away    = getR("road");
      const last10  = getR("lastTen");
      ...
      stats[name] = {
        ...
        streak, abbrev:team.abbreviation, id:team.id,
      };
    }

    console.log(`[ESPN] Fetched records for ${Object.keys(stats).length} teams`);
    _teamStatsCache = stats;
    if(kvSave) await kvSave(stats); // persist to KV
    return stats;
  } catch(e) {
    console.warn("[ESPN] Team stats failed:", e.message);
    return null;
  }
}

async function fetchTeamData(teamName) {
  // Use in-memory cache first (populated by run-engine before buildConvictionPlays)
  if(_teamStatsCache?.[teamName]) return _teamStatsCache[teamName];
  // Fall back to hardcoded standings
  return STANDINGS_FALLBACK[teamName] || { wins:0, losses:0 };
}

// Called by run-engine.js to pre-populate cache before conviction scoring
export function setTeamStatsCache(stats) {
  _teamStatsCache = stats;
}

export async function buildConvictionPlays(games, mlWeights = null) {
  // Use ML-learned weights if available, otherwise use base weights
