...
const MIN_EV_EDGE = 3.5;         // 3.5% min edge — filters noise, keeps real value
const MIN_EV_EDGE_LONGSHOT = 7;  // 7% min on longshots >+125 — higher variance needs more cushion
const MIN_EV_EDGE_PROP = 3.5;    // Match standard threshold for consistency
...
export async function buildConvictionPlays(games, mlWeights = null, teamStats = null) {
  const BASE_WEIGHTS = { winRate:0.22, netRating:0.20, recentForm:0.18, ats:0.14, homeAway:0.12, oppForm:0.08, market:0.06 };
  ...
  for(const game of games) {
    ...
    for(const [team, opp, record, oppRecord, isHome] of [
      ...
      const signals = [
        { key:"winRate",    label:"Season Win Rate",      weight: weights.winRate    || 0.22, score: clamp(winRateScore) },
        { key:"netRating",  label:"Record vs Opponent",   weight: weights.netRating  || 0.20, score: clamp(recordEdgeScore) },
        { key:"recentForm", label:"Recent Form (L10)",    weight: weights.recentForm || 0.18, score: clamp(formScore) },
        { key:"ats",        label:"ATS Tendency",         weight: weights.ats        || 0.14, score: clamp(atsScore) },
        { key:"homeAway",   label:"Home/Away Record",     weight: weights.homeAway   || 0.12, score: clamp(homeScore) },
        { key:"oppForm",    label:"Opponent Form (L10)",  weight: weights.oppForm    || 0.08, score: clamp(oppFormScore) },
        { key:"market",     label:"Market Implied Prob",  weight: weights.market     || 0.06, score: clamp(marketScore) },
      ];
...
export function placeBets(evBets, convictionPlays, currentBankroll, existingHistory) {
  ...
  for(const play of convictionPlays) {
    ...
    const decOdds = americanToDecimal(effectiveOdds);
    const payout = +(wagerAmt * (decOdds - 1)).toFixed(2);
