...
export async function fetchKalshiOdds() {
  try {
    // Use /events with status=open and keyword filter for NBA
    // with_nested_markets=true returns the markets inline
    const url = "https://api.elections.kalshi.com/trade-api/v2/events?limit=100&with_nested_markets=true&status=open";
    const res = await fetch(url, {
      cache: "no-store",
    ...
    // Filter to NBA single-game winner markets
    // Kalshi NBA game titles look like: "Will the Lakers win vs Celtics?"
    // or "Lakers to win?" — we filter by title keywords
    const nbaEvents = events.filter(e => {
      const title = (e.title || e.event_ticker || "").toLowerCase();
      return title.includes("nba") || title.includes("basketball") ||
             title.includes("lakers") || title.includes("celtics") ||
             title.includes("warriors") || title.includes("bulls") ||
             // Kalshi NBA event tickers often start with KXNBA
             (e.event_ticker || "").toUpperCase().startsWith("KXNBA");
    });

    console.log(`[Kalshi] ${events.length} open events, ${nbaEvents.length} NBA`);

    const results = [];
    for (const event of nbaEvents) {
      const markets = event.markets || [];
      for (const m of markets) {
        // We want "will X win?" type markets — yes_bid is the win probability
        if (!m.yes_bid && !m.yes_ask) continue;
        const yesMid = ((m.yes_bid || 0) + (m.yes_ask || m.yes_bid || 0)) / 2;
        if (yesMid <= 2 || yesMid >= 98) continue; // skip near-certain markets
        ...
        // Patterns: "Will the Golden State Warriors win?", "Lakers to win vs Celtics?"
        //           "Golden State Warriors to win?", "Warriors win tonight?"
        let rawTitle = (m.title || event.title || "");
        let teamName = rawTitle
          .replace(/^Will (the )?/i, "")
          .replace(/\s+to win.*$/i, "")   // "X to win?" → "X"
          .replace(/\s+win.*$/i, "")       // "X win?" → "X"
          .replace(/\?$/, "")
          .replace(/^the /i, "")
          .trim();
