...
const SPORTSBOOKS = ["draftkings","fanduel","betmgm","caesars","pointsbet","betrivers","fliff"];
const MIN_EV_EDGE = 3.5;          // +EV bets: 3.5% min edge
const MIN_EV_EDGE_LONGSHOT = 7;   // Longshots >+125: 7% min (higher variance)
const MIN_EV_EDGE_PROP = 3.5;     // Props: same as standard EV
const KELLY_FRACTION = 0.25; // kept for reference; dynamic version used in practice
const CONVICTION_THRESHOLD = 70;

// Upgrade 4 — book sharpness weights for weighted-consensus devig
const SHARP_BOOKS    = ["pinnacle", "betrivers", "lowvig"];
const STANDARD_BOOKS = ["draftkings", "fanduel", "betmgm", "caesars"];
const BOOK_WEIGHTS   = Object.fromEntries([
  ...SHARP_BOOKS.map(b    => [b, 3]),
  ...STANDARD_BOOKS.map(b => [b, 1]),
]);

// Upgrade 3 — dynamic Kelly fraction: high conviction + strong edge → larger sizing
function dynamicKellyFraction(convictionScore, edge) {
  const convBonus  = convictionScore >= 80 ? 0.10 : convictionScore >= 70 ? 0.05 : 0;
  const edgeBonus  = edge >= 0.06 ? 0.05 : edge >= 0.04 ? 0.02 : 0;
  return Math.min(0.35, 0.20 + convBonus + edgeBonus);
}
const MIN_PAYOUT_RATIO = 0.10; // payout must be ≥10% of wager (blocks anything worse than ~-1000)
const MIN_EV_PAYOUT_RATIO = 0.15; // EV bets need ≥15% return (worse than -667 not worth it)
...
// Convert true probability to American odds (the break-even line)
...
// "Get at X or better" — worst odds where bet still has edge >= minEdgePct
// minEdgePct default 0.02 = 2% minimum edge to still be worth taking
function getAtOrBetter(trueProb, minEdgePct = 0.035) { // 3.5% buffer — walk away when edge drops below entry threshold
  if (!trueProb || trueProb <= 0 || trueProb >= 1) return null;
  // Break-even implied prob = trueProb - minEdge buffer
  // i.e. the book's implied prob must be at most (trueProb - minEdge)
  const worstImplied = trueProb - minEdgePct;
  if (worstImplied <= 0 || worstImplied >= 1) return null;
  return probToAmerican(worstImplied);
}
...
function kellyPct(edge, decOdds, fraction) {
  if(edge <= 0 || decOdds <= 1) return 0;
  const f    = fraction !== undefined ? fraction : dynamicKellyFraction(0, edge);
  const full = (edge * decOdds) / (decOdds - 1);
  return Math.min(full * f * 100, 8); // cap at 8%
}

// ── ODDS API ───────────────────────────────────────────────────────────────────

export async function fetchLiveOdds(apiKey) {
  // Uses key rotation — apiKey param kept for backwards compat only
  const books = "draftkings,fanduel,betmgm,caesars,pointsbet,betrivers,lowvig,betonlineag,bovada,mybookieag,betus,pinnacle,fliff";
  const urlTemplate = `https://api.the-odds-api.com/v4/sports/basketball_nba/odds/?regions=us,us2&markets=h2h,spreads,totals&bookmakers=${books}&oddsFormat=american`;
  try {
    const { data } = await fetchOddsAPI(urlTemplate);
    if(!Array.isArray(data)) { console.warn("[OddsAPI] unexpected response"); return null; }
    console.log(`[OddsAPI] ${data.length} games loaded via key rotation`);
    return data;
  } catch(e) {
    console.error("[OddsAPI] fetchLiveOdds failed:", e.message);
    return null;
  }
}
...
export async function fetchScores(apiKey) {
  ...
  try {
    const { data } = await fetchOddsAPI(
      `https://api.the-odds-api.com/v4/sports/basketball_nba/scores/?daysFrom=2`
    );
...
// ── KALSHI INTEGRATION ─────────────────────────────────────────────────────────
// Fetches NBA game winner markets from Kalshi prediction market.
// Uses the /events endpoint with with_nested_markets=true — this is the
// correct Kalshi v2 API pattern (series_ticker filter is not supported on /markets).
// Yes prices (in cents) are converted to American odds and used as:
//   1. A true probability source (cleaner than Pinnacle devig — no math needed)
//   2. A displayed sportsbook line in the odds table alongside DK/FD/etc.
// No API key required for public market data.

export async function fetchKalshiOdds() {
  try {
    // KXNBAGAME is the confirmed series ticker for NBA game-by-game win markets
    // Title format: "Chicago at Los Angeles C", "Minnesota at Golden State"
    // Each event has two nested markets: one for each team (home/away win)
    const url = "https://api.elections.kalshi.com/trade-api/v2/events?series_ticker=KXNBAGAME&with_nested_markets=true&status=open&limit=50";
    const res = await fetch(url, {
      cache: "no-store",
    ...
    if (!res.ok) {
      console.log(`[Kalshi] HTTP ${res.status} — skipping`);
      return [];
    }

    const data = await res.json();
    const events = data.events || [];
    const nbaEvents = events; // all events in this series are NBA games

    console.log(`[Kalshi] ${events.length} NBA game events found`);

    const results = [];
    for (const event of nbaEvents) {
      // KXNBAGAME event title: "Away at Home" e.g. "Chicago at Los Angeles C"
      const eventTitle = event.title || "";
      const titleParts = eventTitle.split(/ at /i);
      const awayName = titleParts[0]?.trim() || "";
      const homeName = titleParts[1]?.trim() || "";

      const markets = event.markets || [];
      for (const m of markets) {
        if (!m.yes_bid && !m.yes_ask) continue;
        const yesMid = ((m.yes_bid || 0) + (m.yes_ask || m.yes_bid || 0)) / 2;
        if (yesMid <= 2 || yesMid >= 98) continue;

        const yesProb = yesMid / 100;
        const noProb  = 1 - yesProb;
        const yesOdds = yesProb > 0.5
          ? Math.round(-(yesProb / noProb) * 100)
          : Math.round((noProb / yesProb) * 100);
        const noOdds = noProb > 0.5
          ? Math.round(-(noProb / yesProb) * 100)
          : Math.round((yesProb / noProb) * 100);

        // Identify which team this market is for using ticker suffix
        // Ticker: KXNBAGAME-26MAR12CHILAC-CHI → last segment = "CHI"
        const suffix = (m.ticker || "").split("-").pop()?.toLowerCase() || "";
        const awaySnip = awayName.toLowerCase().replace(/\s+/g, "").slice(0, 3);
        const teamName = (suffix && awaySnip && suffix.startsWith(awaySnip))
          ? awayName
          : homeName;
    ...
    console.log(`[Kalshi] Parsed ${results.length} team win markets`);
    return results;
  } catch (e) {
    console.log(`[Kalshi] Fetch failed: ${e.message} — skipping`);
    return [];
  }
}

export function mergeKalshiIntoGames(games, kalshiMarkets) {
  if (!kalshiMarkets?.length) return games;

  return games.map(game => {
    const homeTeam = game.home_team || "";
    const awayTeam = game.away_team || "";

    // Skip Kalshi injection for games that have already tipped off —
    // Kalshi reprices live so comparing to stale pregame sportsbook lines is invalid
    const gameStart = game.commence_time ? new Date(game.commence_time) : null;
    if (gameStart && gameStart < new Date()) return game;

    // Find Kalshi markets for home and away teams
    // Match on: last word, any word overlap, or full name substring
    function kalshiTeamMatch(kalshiName, oddsTeam) {
      const k = kalshiName.toLowerCase().trim();
      const o = oddsTeam.toLowerCase().trim();
      if (k === o) return true;
      if (k.includes(o) || o.includes(k)) return true;
      // Match on any meaningful word (skip short words)
      const kWords = k.split(/\s+/).filter(w => w.length >= 4);
      const oWords = o.split(/\s+/).filter(w => w.length >= 4);
      return kWords.some(kw => oWords.some(ow => kw === ow || kw.includes(ow) || ow.includes(kw)));
    }

    const homeMarket = kalshiMarkets.find(k => kalshiTeamMatch(k.teamName, homeTeam));
    const awayMarket = kalshiMarkets.find(k => kalshiTeamMatch(k.teamName, awayTeam));

    if (!homeMarket && !awayMarket) return game;

    // Build a synthetic bookmaker entry shaped like Odds API bookmakers
    const kalshiBookmaker = {
      key: "kalshi",
      title: "Kalshi",
      markets: [
        {
          key: "h2h",
          outcomes: [
            homeMarket ? { name: homeTeam, price: homeMarket.yesOdds } : null,
            awayMarket ? { name: awayTeam, price: awayMarket.yesOdds } : null,
          ].filter(Boolean),
        }
      ],
      // Store raw probs for devig use
      kalshiProbs: {
        [homeTeam]: homeMarket?.yesProb || null,
        [awayTeam]: awayMarket?.yesProb || null,
      }
    };

    return {
      ...game,
      bookmakers: [...(game.bookmakers || []), kalshiBookmaker],
    };
  });
}
...
export function extractEVBets(games) {
  ...
  for(const game of games) {
    ...
    function getTrueProbs_h2h() {
      ...
      // Fallback: weighted consensus — sharp books (Pinnacle/BetRivers/LowVig) get 3x weight
      const homeImp = [], awayImp = [];
      for(const bk of bookmakers) {
        ...
        if(!h || !a) continue;
        const w = BOOK_WEIGHTS[bk.key] || 0.5;
        homeImp.push({ imp: americanToImplied(h.price), w });
        awayImp.push({ imp: americanToImplied(a.price), w });
      }
      if(!homeImp.length) return null;
      const totalW  = homeImp.reduce((s,x) => s + x.w, 0);
      const avgHImp = homeImp.reduce((s,x) => s + x.imp * x.w, 0) / totalW;
      const avgAImp = awayImp.reduce((s,x) => s + x.imp * x.w, 0) / totalW;
      const tot = avgHImp + avgAImp;
      return { home: avgHImp / tot, away: avgAImp / tot };
    }
    ...
    function getTrueProbs_total() {
      ...
      // Fallback: weighted consensus
      const overImp = [], underImp = [];
      for(const bk of bookmakers) {
        if(bk.key === "pinnacle") continue;
        ...
        if(!o || !u) continue;
        const w = BOOK_WEIGHTS[bk.key] || 0.5;
        overImp.push({ imp: americanToImplied(o.price), w });
        underImp.push({ imp: americanToImplied(u.price), w });
      }
      if(!overImp.length) return null;
      const totalWt  = overImp.reduce((s,x) => s + x.w, 0);
      const avgOImp  = overImp.reduce((s,x) => s + x.imp * x.w, 0) / totalWt;
      const avgUImp  = underImp.reduce((s,x) => s + x.imp * x.w, 0) / totalWt;
      const tot2 = avgOImp + avgUImp;
      return { home: avgOImp / tot2, away: avgUImp / tot2 };
    }
    ...
    function pushBet(bet) {
      ...
      // Sanity check: reject if bestOdds is a huge outlier vs consensus
      // e.g. one book at +225 while all others are -300 = stale/error line
      if(allLines && Object.keys(allLines).length >= 3) {
        const allOdds = Object.values(allLines).map(v => typeof v === "object" ? v.odds : v);
        const avgOdds = allOdds.reduce((s,o) => s + o, 0) / allOdds.length;
        // If best odds are 200+ points better than average, likely a stale outlier
        if(bestOdds - avgOdds > 200) return;
        // If true prob says team is 60%+ favorite but best odds are +200, skip
        if(bet.trueProb > 0.60 && bestOdds > 150) return;
      }

      const decOdds = americanToDecimal(bestOdds);
      const kPct = kellyPct(edge, decOdds, dynamicKellyFraction(0, edge));
      if(kPct <= 0) return;
      bets.push({
        ...bet,
    ...
    if(mlProbs) {
      for(const [team, trueProb] of [
        ...
        pushBet({
          ...
          getAtOrBetter: getAtOrBetter(trueProb),
    ...
    // ── 2. SPREADS — devig from spread market itself, not moneyline ────────
    {
      ...
      // Fallback: weighted consensus spread devig
      if(!spreadProbs[game.home_team]) {
        const hSpreadImp = [], aSpreadImp = [];
        for(const bk of bookmakers) {
          if(bk.key === "pinnacle") continue;
          ...
          if(!h || !a) continue;
          const w = BOOK_WEIGHTS[bk.key] || 0.5;
          hSpreadImp.push({ imp: americanToImplied(h.price), w });
          aSpreadImp.push({ imp: americanToImplied(a.price), w });
        }
        if(hSpreadImp.length >= 2) {
          const sw    = hSpreadImp.reduce((s,x) => s + x.w, 0);
          const avgHI = hSpreadImp.reduce((s,x) => s + x.imp * x.w, 0) / sw;
          const avgAI = aSpreadImp.reduce((s,x) => s + x.imp * x.w, 0) / sw;
          const tot3  = avgHI + avgAI;
          spreadProbs[game.home_team] = avgHI / tot3;
          spreadProbs[game.away_team] = avgAI / tot3;
        }
      }
...
// ── INJURY & REST DATA PIPELINE ───────────────────────────────────────────────
async function fetchInjuryAndRestData() {
  const result = {};
  const makeDateStr = d => new Date(Date.now() - d * 86400000).toISOString().slice(0,10).replace(/-/g,"");

  // Build a map of team → date of most recent completed game (last 2 days)
  const teamLastGame = {};
  for(const daysAgo of [1, 2]) {
    try {
      const res = await fetch(
        `https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard?dates=${makeDateStr(daysAgo)}`,
        { cache:"no-store" }
      );
      if(!res.ok) continue;
      const data = await res.json();
      for(const event of (data.events || [])) {
        ...
        if(!state?.completed && state?.state !== "post") continue;
        const gameDate = new Date(event.date);
        for(const competitor of (comp?.competitors || [])) {
          const name = competitor?.team?.displayName;
          if(!name) continue;
          if(!teamLastGame[name] || gameDate > teamLastGame[name]) teamLastGame[name] = gameDate;
        }
      }
    } catch(e) { /* skip */ }
  }

  // Today's scoreboard — injuries + rest calculation
  try {
    const res = await fetch(
      "https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard",
      { cache:"no-store" }
    ...
    for(const event of (data.events || [])) {
      ...
      for(const competitor of (comp?.competitors || [])) {
        ...
        // Key player injuries (G/F positions)
        const injuries = competitor.injuries || [];
        const keyInjuries = injuries.filter(inj => {
          const s   = inj.status || "";
          const pos = inj.athlete?.position?.abbreviation || "";
          return (s === "Out" || s === "Doubtful") &&
                 ["G","F","PG","SG","SF","PF"].includes(pos);
        });

        // Rest days from last game
        const lastGame = teamLastGame[name];
        const daysRest = lastGame ? (gameDate - lastGame) / 86400000 : 2;

        result[name] = {
          injuryRisk:     keyInjuries.length > 0,
          injuredPlayers: keyInjuries.map(i => i.athlete?.displayName).filter(Boolean),
          backToBack:     daysRest < 1.5,
          wellRested:     daysRest >= 3,
          daysRest:       +daysRest.toFixed(1),
        };
      }
    }
  } catch(e) { console.warn("[InjuryRest] failed:", e.message); }
  return result;
}

export async function buildConvictionPlays(games, mlWeights = null, teamStats = null) {
  // 9-signal normalized weights (pre-normalization: winRate:0.20, netRating:0.18,
  // recentForm:0.16, ats:0.12, homeAway:0.10, oppForm:0.07, market:0.05,
  // rest:0.10, injury:0.08 → sum=1.06 → divide each by 1.06)
  const BASE_WEIGHTS = {
  ...
  // ML Upgrade 1 — injury + rest data
  const injuryRestData = await fetchInjuryAndRestData();
  ...
  for(const game of games) {
    ...
    for(const [team, opp, record, oppRecord, isHome] of [
      ...
      // 8. Rest Advantage — well-rested vs opponent on back-to-back
      const teamRest = injuryRestData[team] || {};
      const oppRest  = injuryRestData[opp]  || {};
      let restScore = 50;
      if(teamRest.backToBack) restScore = 20;
      else if(teamRest.wellRested) restScore = 80;
      if(oppRest.backToBack) restScore = Math.min(98, restScore + 15); // opponent tired = bonus
      const restAdvantage = teamRest.wellRested && !oppRest.wellRested;

      // 9. Injury Report — opponent key injuries = advantage; own key injuries = penalty
      const teamInjured = teamRest.injuryRisk || false;
      const oppInjured  = oppRest.injuryRisk  || false;
      let injuryScore = 50;
      if(oppInjured && !teamInjured) injuryScore = 85;   // we're healthy, they're not
      else if(teamInjured && !oppInjured) injuryScore = 25; // we're hurt, they're not
      const injuryAdvantage = oppInjured && !teamInjured;

      const clamp = (v) => Math.max(5, Math.min(98, Math.round(v)));

      const signals = [
        { key:"winRate",    label:"Season Win Rate",      weight: weights.winRate    || 0.1887, score: clamp(winRateScore) },
        { key:"netRating",  label:"Record vs Opponent",   weight: weights.netRating  || 0.1698, score: clamp(recordEdgeScore) },
        { key:"recentForm", label:"Recent Form (L10)",    weight: weights.recentForm || 0.1509, score: clamp(formScore) },
        { key:"ats",        label:"ATS Tendency",         weight: weights.ats        || 0.1132, score: clamp(atsScore) },
        { key:"homeAway",   label:"Home/Away Record",     weight: weights.homeAway   || 0.0943, score: clamp(homeScore) },
        { key:"oppForm",    label:"Opponent Form (L10)",  weight: weights.oppForm    || 0.0660, score: clamp(oppFormScore) },
        { key:"market",     label:"Market Implied Prob",  weight: weights.market     || 0.0472, score: clamp(marketScore) },
        { key:"rest",       label:"Rest Advantage",       weight: weights.rest       || 0.0943, score: clamp(restScore) },
        { key:"injury",     label:"Injury Report",        weight: weights.injury     || 0.0755, score: clamp(injuryScore) },
      ];
      ...
      plays.push({
        ...
        // Injury + rest flags (ML Upgrade 1)
        backToBack: teamRest.backToBack || false,
        wellRested: teamRest.wellRested || false,
        daysRest: teamRest.daysRest || null,
        injuryRisk: teamInjured,
        restAdvantage,
        injuryAdvantage,
        injuredPlayers: teamRest.injuredPlayers || [],
      });
    }
  }
...
export function placeBets(evBets, convictionPlays, currentBankroll, existingHistory) {
  ...
  for(const bet of evBets) {
    ...
    if(payout / wagerAmt < MIN_EV_PAYOUT_RATIO) { console.log(`[Engine] Skipping EV bet ${bet.selection} — payout ratio ${(payout/wagerAmt*100).toFixed(1)}% below ${MIN_EV_PAYOUT_RATIO*100}% min`); continue; }
    newEntries.push({
  ...
  for(const play of convictionPlays) {
    ...
    // If no market odds available, estimate from our win probability
    // e.g. 60% win prob → -150, 40% → +150
    const effectiveOdds = play.bestOdds || (() => {
      const p = (play.ourProbability || 50) / 100;
      return p > 0.5 ? Math.round(-(p / (1-p)) * 100) : Math.round(((1-p) / p) * 100);
    })();
    const effectiveBook = play.bestBook || "estimated";
    // ML Upgrade 3 — dynamic Kelly for conviction bets
    const edge = play.ourProbability ? ((play.ourProbability / 100) - americanToImplied(effectiveOdds)) : 0;
    const kellyFrac = dynamicKellyFraction(play.convictionScore || 0, Math.max(0, edge));
    const decOddsConv = americanToDecimal(effectiveOdds);
    const fullKelly = edge > 0 ? (edge * decOddsConv) / (decOddsConv - 1) : 0;
    const dynKellyPct = Math.min(fullKelly * kellyFrac * 100, 8);
    const convKellyPct = Math.max(dynKellyPct, 1.5); // floor at 1.5%
    const wagerAmt = +(bankroll * (convKellyPct / 100)).toFixed(2);
    const decOdds = decOddsConv;
    const payout = +(wagerAmt * (decOdds - 1)).toFixed(2);
    if(payout / wagerAmt < MIN_PAYOUT_RATIO) { console.log(`[Engine] Skipping conviction bet ${play.selection} — payout ratio ${(payout/wagerAmt*100).toFixed(1)}% below ${MIN_PAYOUT_RATIO*100}% min`); continue; }
    newEntries.push({
      ...
      bestOdds: effectiveOdds, bestBook: effectiveBook,
      kellyPct: +convKellyPct.toFixed(2),
      wagerAmt, potentialPayout: payout,
      ev: null, edge: null,
      ourProbability: play.ourProbability,
      convictionScore: play.convictionScore,
      getAtOrBetter: getAtOrBetter((play.ourProbability || 50) / 100),
      gameTime: play.gameTime,
      status: "pending", bankrollBefore: +bankroll.toFixed(2),
      bankrollAfter: +bankroll.toFixed(2), result: null,
      isConviction: true,
      oddsEstimated: !play.bestOdds,
    });
...
export function resolveHistory(history, scores) {
  ...
  const updated = history.map(entry => {
    ...
    const betDateStr = new Date(entry.gameTime || entry.date).toDateString();

    // Try matching by game string first, then fall back to just team names in selection
    // IMPORTANT: only match games from the same calendar date as the bet was placed
    const gameScore = scores.find(s =>
      normTeamMatch(entry.game, s.home_team || "", s.away_team || "")
      && new Date(s.commence_time || Date.now()).toDateString() === betDateStr
    ) || scores.find(s => {
      // Fallback: match any team in the selection against score teams — same date only
      if (new Date(s.commence_time || Date.now()).toDateString() !== betDateStr) return false;
      const sel = entry.selection?.toLowerCase() || "";
      const h = normTeam(s.home_team), a = normTeam(s.away_team);
      return sel.includes(h) || sel.includes(a);
    });

    if(!gameScore || !gameScore.completed) {
      // Only estimate after 8hrs (games take ~3hrs, give buffer for late games)
      if(isMockId || gameAge > 8) {
    ...
    // ML Upgrade 2 — CLV: compare entry odds vs fair-value closing line
    let clv = null;
    if(entry.bestOdds != null && entry.ourProbability != null) {
      const fairLine = probToAmerican(entry.ourProbability / 100);
      if(fairLine != null) clv = entry.bestOdds - fairLine;
    }
