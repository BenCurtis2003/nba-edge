...
export async function fetchKalshiOdds() {
  try {
    ...
    for (const event of nbaEvents) {
      // KXNBAGAME event title format: "Chicago at Los Angeles C" (away at home)
      // Each event has 2 nested markets — one per team win contract
      // Market ticker format: KXNBAGAME-26MAR12CHILAC-CHI, KXNBAGAME-26MAR12CHILAC-LAC
      // Market title is the team's short city/nickname
      ...
      for (const m of markets) {
        ...
        if (yesMid <= 2 || yesMid >= 98) continue; // skip near-certain/dead markets
        ...
        // Team name: use market title if it exists (short city name like "Chicago")
        // Fall back to parsing the event title "Away at Home" split
        let teamName = (m.title || "").trim();

        // If market title is unhelpful (same as event title), parse event title
        if (!teamName || teamName === eventTitle || teamName.length < 3) {
          // "Chicago at Los Angeles C" → ["Chicago", "Los Angeles C"]
          const parts = eventTitle.split(/ at /i);
          // Use ticker suffix to figure out which team this market is for
          // Ticker: KXNBAGAME-26MAR12CHILAC-CHI → suffix = CHI → away team
          const suffix = (m.ticker || "").split("-").pop()?.toLowerCase() || "";
          const awaySnip = parts[0]?.trim().toLowerCase().slice(0, 4) || "";
          teamName = awaySnip && suffix.startsWith(awaySnip) ? parts[0]?.trim() : parts[1]?.trim();
        }
