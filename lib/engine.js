  ...
  try {
    ...
    for (const event of nbaEvents) {
      ...
      for (const m of markets) {
        ...
        let teamName = (m.title || event.title || "")
          .replace(/^Will (the )?/i, "")
          .replace(/\s+win.*$/i, "")
          .replace(/\?$/, "")
          .trim();
...
export function mergeKalshiIntoGames(games, kalshiMarkets) {
  ...
  return games.map(game => {
    ...
    const homeMarket = kalshiMarkets.find(k => {
      const kName = k.teamName.toLowerCase();
      const hName = homeTeam.toLowerCase();
      // Match on last word (e.g. "Warriors" in "Golden State Warriors")
      const kLast = kName.split(" ").pop();
      const hLast = hName.split(" ").pop();
      return kLast === hLast || kName.includes(hLast) || hName.includes(kLast);
    });

    const awayMarket = kalshiMarkets.find(k => {
      const kName = k.teamName.toLowerCase();
      const aName = awayTeam.toLowerCase();
      const kLast = kName.split(" ").pop();
      const aLast = aName.split(" ").pop();
      return kLast === aLast || kName.includes(aLast) || aName.includes(kLast);
    });
