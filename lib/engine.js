...
//   1. A true probability source (like Pinnacle devig, but cleaner)
//   2. A displayed sportsbook line in the odds table
// No API key required for public market data.

export async function fetchKalshiOdds() {
  try {
    // Fetch NBA game markets — series ticker KXNBA covers game winner markets
    const res = await fetch(
      "https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=KXNBA&limit=100&status=open",
    ...
    console.log(`[Kalshi] ${markets.length} open NBA markets`);

    // Parse into a format we can merge with Odds API games
    // Each market title looks like: "Warriors to win?" or "OKC Thunder to win vs Lakers?"
    const results = [];
    for (const m of markets) {
      if (!m.title || m.yes_bid == null) continue;

      // Mid price = average of bid/ask → best estimate of true probability
      const yesMid = ((m.yes_bid || 0) + (m.yes_ask || m.yes_bid || 0)) / 2;
      if (yesMid <= 0 || yesMid >= 100) continue;
      ...
      // Extract team name from title — "Golden State Warriors to win" → "Golden State Warriors"
      const titleClean = m.title
        .replace(/\s+to win.*$/i, "")
        .replace(/Will the /i, "")
        .replace(/\?$/, "")
        .trim();
      ...
      results.push({
        ticker: m.ticker,
        teamName: titleClean,
        yesPrice: yesMid,      // cents, e.g. 58 = 58% probability
        yesProb,               // 0-1
        yesOdds,               // American odds for yes (team wins)
        noOdds,                // American odds for no (team loses)
        volume: m.volume || 0,
        closeTime: m.close_time,
      });
    }

    console.log(`[Kalshi] Parsed ${results.length} team markets`);
    return results;
  } catch (e) {
...
// Merge Kalshi odds into the games array from Odds API.
// Adds a synthetic "kalshi" bookmaker entry to each matching game.
