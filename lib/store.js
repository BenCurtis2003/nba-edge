...
function defaultML() {
  return {
    ...
    // Signal keys match engine signal keys exactly
    signalAccuracy: {
...
export async function updateMLAfterResolution(resolvedBets) {
  ...
  try {
    ...
    if(convictionBets >= (ml.minBetsForML || 30)) {
      const baseWeights = { winRate:0.22, netRating:0.20, recentForm:0.18, ats:0.14, homeAway:0.12, oppForm:0.08, market:0.06 };
      const accuracies = {};
