...
export async function fetchOddsAPI(urlTemplate, options = {}) {
  ...
  let lastError = null;
  const allExhausted = rotation.every(e => e.remaining < MIN_REMAINING);
  if (allExhausted) {
    console.warn("[OddsKeys] All keys at/near 0 — attempting anyway in case quota reset");
  }
  for (const { key, short, remaining } of rotation) {
    if (!allExhausted && remaining < MIN_REMAINING) {
