      ...
      {chartData.length > 1 && (
        <div style={s.section}>
          <div style={{background:"#0a1220", border:"1px solid #172030", borderRadius:12, padding:"20px 24px"}}>
            <div style={{fontSize:13, fontWeight:700, color:"#fff", marginBottom:4}}>Portfolio Performance</div>
            <div style={{fontSize:10, color:"#3a5570", marginBottom:16}}>
              $100 starting bankroll · Kelly Criterion sizing · Bayesian ML
            </div>
          ...
          {filteredBets.length === 0 ? (
            ...
            <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))", gap:12}}>
              {filteredBets.map(bet => (
                <div key={bet.id} style={{background:"#0a1220", border:"1px solid #172030",
                  borderRadius:12, padding:"16px"}}>
                  <div style={{fontSize:13, fontWeight:700, color:"#fff", marginBottom:3}}>
                    {bet.selection}
                  </div>
                  <div style={{fontSize:10, color:"#3a5570", marginBottom:8}}>{bet.game}</div>
                  <div style={{display:"flex", gap:6, marginBottom:10, flexWrap:"wrap"}}>
                    <span style={s.badge("#00bfff")}>{bet.type}</span>
                    <span style={{fontSize:11, fontWeight:700,
                      color:bet.bestOdds<0?"#00bfff":"#ffd700"}}>{formatOdds(bet.bestOdds)}</span>
                    <span style={{fontSize:9, color:SPORTSBOOK_COLORS[bet.bestBook]||"#3a5570"}}>{bet.bestBook}</span>
                  </div>
                  <div style={{display:"flex", gap:12, fontSize:10}}>
                    <div>Edge <span style={{color:"#00ff88", fontWeight:600}}>+{bet.edge?.toFixed(1)}%</span></div>
                    <div>EV <span style={{color:"#00ff88", fontWeight:600}}>+{bet.ev?.toFixed(1)}%</span></div>
                    <div>Kelly <span style={{color:"#b44fff", fontWeight:600}}>{bet.kellyPct?.toFixed(1)}%</span></div>
                  </div>
                </div>
              ))}
            </div>
