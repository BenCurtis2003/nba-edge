...
const SPORTSBOOK_COLORS = {
  ...
  lowvig:"#ff9944", betonlineag:"#ff6b6b", bovada:"#ffd700",
  mybookieag:"#b44fff", betus:"#00bfff", pinnacle:"#00ff88",
};
...
const s = {
  ...
  pill: { fontSize:9, padding:"2px 10px", borderRadius:20, border:"1px solid #172030", color:"#3a5570" },
  ...
  statGrid: { display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:12, padding:"24px 32px" },
      ...
      {expanded && (
        <div style={{marginTop:14, borderTop:"1px solid #0e1a28", paddingTop:14}}>
          ...
          <div style={{background:"#060d16", borderRadius:8, padding:"10px 12px",
            ...
            True win probability ({bet.ourProbability?.toFixed(1)}%) exceeds book implied odds ({bet.bookImplied?.toFixed(1)}%),
            giving a {bet.edge?.toFixed(1)}% edge. At {formatOdds(bet.bestOdds)} on {bet.bestBook},
            this is a {bet.ev?.toFixed(1)}% EV bet. Kelly recommends {bet.kellyPct?.toFixed(1)}% of bankroll.
          </div>
        </div>
      )}
    </div>
    ...
    <div style={{...s.convCard, cursor:"pointer", borderColor:isAutoBet?"#00ff8822":"#172030"}}
      ...
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8}}>
        <div style={{display:"flex", gap:6, flexWrap:"wrap"}}>
          ...
          {play.usingMLWeights && <span style={s.badge("#b44fff")}>🤖 ML</span>}
        </div>
      ...
      <div style={{display:"flex", gap:8, marginBottom:10}}>
        ...
        <span style={{fontSize:10, color:"#3a5570", padding:"2px 8px", borderRadius:4}}>vs {play.oppRecord}</span>
      </div>
      {play.bestOdds && (
      ...
      {expanded && play.signals && (
        <div style={{marginTop:12, borderTop:"1px solid #0e1a28", paddingTop:12}}>
          <div style={{fontSize:9, color:"#3a5570", marginBottom:8, letterSpacing:"0.08em"}}>SIGNAL BREAKDOWN</div>
            ...
            <div key={sig.key} style={{marginBottom:6}}>
              <div style={{display:"flex", justifyContent:"space-between", marginBottom:2}}>
                ...
                <span style={{fontSize:9, fontWeight:700,
                  color:sig.score>=70?"#00ff88":sig.score>=55?"#ffd700":"#ff6b6b"}}>{sig.score}/100</span>
              </div>
              <div style={{height:2, background:"#0e1a28", borderRadius:1}}>
                <div style={{height:"100%", width:`${sig.score}%`, borderRadius:1,
                  background:sig.score>=70?"#00ff88":sig.score>=55?"#ffd700":"#ff6b6b", opacity:0.7}}/>
              </div>
            </div>
          ))}
        </div>
    ...
    <div style={{borderBottom:"1px solid #0e1a28",
      ...
      <div style={{display:"grid", gridTemplateColumns:"100px 1fr auto", gap:12,
        ...
        <div style={{textAlign:"right", minWidth:110}}>
          {pnl!==null&&<div style={{fontSize:18,fontWeight:800,color:accentColor}}>{pnl>0?"+":""}{fmt$(pnl)}</div>}
          <div style={{fontSize:9,color:"#3a5570",marginTop:2}}>Wagered <span style={{color:"#ffd700"}}>{fmt$(h.wagerAmt)}</span></div>
          <div style={{fontSize:9,color:"#3a5570"}}>
...
export default function App() {
  ...
  useEffect(() => {
    fetchPortfolio();
    const iv = setInterval(fetchPortfolio, 60000);
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <span style={{fontSize:10,color:"#3a5570"}}>Updated {timeAgo(data?.lastRun)}</span>
          </div>
      ...
      <div style={s.statGrid}>
        <StatCard label="PAPER BANKROLL" value={`$${(data?.bankroll||100).toFixed(2)}`}
          sub={`${(data?.totalPnl||0)>=0?"+":""}$${(data?.totalPnl||0).toFixed(2)} P&L`} color="#00ff88"/>
        <StatCard label="WIN RATE" value={`${data?.winRate||0}%`}
          sub={`${data?.record?.wins||0}W / ${data?.record?.losses||0}L`} color="#00bfff"/>
        <StatCard label="ROI" value={`${(data?.roi||0)>=0?"+":""}${data?.roi||0}%`}
          sub="on resolved bets" color={(data?.roi||0)>=0?"#00ff88":"#ff6b6b"}/>
        <StatCard label="BETS TODAY" value={currentBets.length}
          sub={`${conviction.filter(p=>p.convictionScore>=70).length} auto-bet conviction plays`} color="#ffd700"/>
        <StatCard label="ML ENGINE" value={data?.mlStatus||"Learning"}
          sub={`${data?.mlBets||0} bets analyzed`} color="#b44fff"/>
      </div>
      ...
      {(tab==="All"||tab==="Conviction") && conviction.length > 0 && (
        <div style={s.section}>
          ...
          <div style={{fontSize:11,color:"#3a5570",marginBottom:14}}>
            Picks based on live team records, home/away splits, recent form & matchup data. Plays ≥70/100 auto-bet.
          </div>
        ...
        <div style={s.section}>
          ...
          {filteredBets.length===0 ? (
            <div style={{color:"#3a5570",fontSize:12,padding:"20px 0"}}>
              {currentBets.length===0?"Engine runs every 8 minutes. Check back soon.":"No bets match this filter."}
            </div>
          ) : (
